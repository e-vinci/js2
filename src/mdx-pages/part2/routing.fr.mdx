...
Voici comment définir la route `/` pour le squelette de l'application qui se trouvera dans `App`, puis les 3 routes "enfants" pour les 3 pages (veuillez mettre à jour le router au sein de `App`) :
...
Nous vous demandons : 
- De mettre à jour la `HomePage` afin qu'elle n'affiche qu'une liste reprenant les titres de vos films favoris.
