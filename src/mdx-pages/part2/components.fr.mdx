...
- à un body repris comme un un élément Flexbox faisant 100% de hauteur.
...
Veuillez créer un nouveau projet en utilisant les technos Vite + React + TS + SWC nommé `/exercises/2.1-2.2` dans votre git repo.
...
Nous allons continuer le projet de votre exercice précédent qui se trouve dans le dossier `/exercises/2.1-2.2` dans votre repo git.
