...
Pour ce tutoriel, veuillez créer une copie du tutoriel `start-state`, si nécessaire voici le code du tutoriel [start-state](https://github.com/e-vinci/ts-demos/tree/main/front/start-state), et l'appeler `medium-state`. Changer le nom du projet dans `package.json`.
