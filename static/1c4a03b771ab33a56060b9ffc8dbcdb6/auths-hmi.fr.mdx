---
title: "Sessions & JWT"
description: "Sessions, authentification & utilisation d'opérations protégées par JWT au sein d'une application React"
---
...
<InternalPageMenu>
  ...
  <InternalPageMenuItem> Exercice 3.3 : Persistence d'un thème  </InternalPageMenuItem>
  <InternalPageMenuItem> Authentification d'un utilisateur via une IHM & JWT </InternalPageMenuItem> 
  <InternalPageMenuItem> Sauvegarde des données de session </InternalPageMenuItem>
  <InternalPageMenuItem> Déconnexion d'un utilisateur </InternalPageMenuItem>
  <InternalPageMenuItem> Autorisation de l'appel à une opération protégée </InternalPageMenuItem>
  <InternalPageMenuItem> Exercice 3.4 : Authentification et appel d'opérations protégées par JWT </InternalPageMenuItem>
  <InternalPageMenuItem> 🍬 Challenge optionnel : Gestion de session & remember me </InternalPageMenuItem>
...
# <InternalPageTitle> Exercice 3.3 : Persistence d'un thème </InternalPageTitle>

Veuillez partir d'une copie de l'exercice (`/exercises/2.15`) pour créer un nouveau projet nommé `exercises/3.3-4` afin de continuer l'application `myMovies`.
...
Pour ce tutoriel, veuillez créer une copie du tutoriel `fetch-proxy`, si nécessaire voici le code du tutoriel [fetch-proxy](https://github.com/e-vinci/ts-demos/tree/main/front/fetch-proxy), et l'appeler `session-jwt`. Changez le nom du projet dans `package.json`.

Veuillez démarrer la version [auths](https://github.com/e-vinci/ts-demos/tree/main/back/api/auths) de l'API de la pizzeria.
...
# <InternalPageTitle> Sauvegarde des données de session </InternalPageTitle>
...
# <InternalPageTitle> Déconnexion d'un utilisateur </InternalPageTitle>
...
# <InternalPageTitle> Exercice 3.4 : Authentification et appel d'opérations protégées par JWT  </InternalPageTitle>

Veuillez continuer l'exercice précédent nommé `/exercises/3.3-4` afin de compléter l'application `myMovies`.
...
Par exemple, vous pourriez afficher des boutons de type **`Effacer`** ou **`Sauver`** que si l'utilisateur est authentifié.  
...
# <InternalPageTitle> 🍬 Challenge optionnel : Gestion de session & remember me  </InternalPageTitle>
...
Dans la partie optionnelle sur l'[Authentification & autorisation JWT à l'aide de cookies](../auths-api/#🍬_authentification_autorisation_jwt_a_laide_de_cookies), nous avons vu comment mettre à jour l'API afin d'intégrer les tokens JWT aux cookies.

Veuillez démarrer la version [cookies](https://github.com/e-vinci/ts-demos/tree/main/back/api/cookies) de l'API de la pizzeria.
...
Pour ce tutoriel, veuillez créer une copie du tutoriel `session-jwt`, si nécessaire voici le code du tutoriel [session-jwt](https://github.com/e-vinci/ts-demos/tree/main/front/session-jwt), et l'appeler `cookies`. Changez le nom du projet dans `package.json`.
