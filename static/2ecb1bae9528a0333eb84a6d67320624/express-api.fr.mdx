...
<InternalPageMenu>
  ...
  <InternalPageMenuItem> Démarrage d'une RESTful API en Express & TS </InternalPageMenuItem>
...
# <InternalPageTitle> Démarrage d'une RESTful API en Express & TS  </InternalPageTitle>
...
Dans ce répertoire, veuillez générer une application express nommée **basic** sur base du boilerplate : [basic-ts-api-boilerplate](https://github.com/e-vinci/basic-ts-api-boilerplate) [[R.51]](/references/#r51).  
Pour ce faire :
...
Veuillez faire un **`commit`** de votre code avec le message suivant : **`new: ex1.1`**.
...
Le code de votre application doit se trouver dans votre repo git au sein du répertoire **`/exercises/1.2`**.  
...
Vous allez maintenant catégoriser le nombre d'appels par **`PATH`** et par **`méthode HTTP`**.
...
Pour consommer cette nouvelle opération via un browser, nous pouvons lire la ressource de type "drinks" identifiée par **2** dans le menu ainsi :
...
Pour consommer cette nouvelle opération via un browser, nous pouvons lire toutes les ressources de type "drinks" triées par leur titre de manière descendante :
...
Ici, le linter ESLint nous avertit que **`req.body`** n'est pas défini, nous avons une allocation de valeurs de type **`any`** insécuritaire. En effet, `title`, `image`, `volume` et `price` peuvent être de n'importe quel type (**`any`**). 
...
Et voici le code de **`/routes/drinks.ts`** mis à jour :
```ts
router.post("/", (req, res) => {
...
Nous avons ajouté une **assertion de type** : grâce à **`as NewDrink`**, nous avons indiqué à TypeScript que **`req.body`** est de type **`NewDrink`**.

Cela peut être utilisé généralement quand on est sûr que le type est correct, mais que TypeScript ne peut pas le déterminer automatiquement... Cela règle les problèmes de compilation.

Par contre, cela ne règle pas les problèmes de runtime. Si le type n'est pas correct, une erreur sera levée lors de l'exécution de l'application.

Pour régler les problèmes à l'exécution aussi, il faut faire ce que l'on appelle du **type narrowing**, ou de la **réduction de type**. Cela consiste à vérifier que les propriétés sont bien définies et qu'elles sont du bon type, généralement à l'aide d'opérateurs (comme **`typeof`** ou **`instanceof`**, les opérateurs d'égalité, l'opérateur **`in`** ) qui réduisent dynamiquement le type de la variable.

Nous serions réellement sûr si nous pouvions exécuter ce code dynamiquement tout en étant sûr que le type est correct : `const { title, image, volume, price } : NewDrink  = req.body;`.

Voici le code de **`/routes/drinks.ts`** mis à jour avec du type narrowing :
```ts
router.post("/", (req, res) => {
  const { title, image, volume, price } = req.body as NewDrink;
...
Au sein de **`drinks.http`**, veuillez ajouter ces deux requêtes pour la lecture d'une seule boission ou pour la lecture de toutes les boissons en les filtrant selon le budget maximum :
```http
### Read a single drink
...
Au sein de **`drinks.http`**, veuillez ajouter cette requête pour la création d'une boission :
```http
### Create a drink
POST {{baseUrl}}/drinks
...
Le code de votre application doit se trouver dans votre repo git dans **`/exercises/1.3`**.  
Veuillez faire un **`commit`** de votre code avec le message suivant : **`new: ex1.3`**.
...
Le code de votre application doit se trouver dans votre repo git dans **`/exercises/1.5`**.  
...
En cas de souci, vous pouvez accéder au code du tutoriel ici : [basic](https://github.com/e-vinci/ts-demos/tree/main/back/api/basic).
...
Le code de votre application doit se trouver dans votre repo git dans **`/exercises/1.6`**.  
