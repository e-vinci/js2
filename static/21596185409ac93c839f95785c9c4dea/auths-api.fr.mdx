...
description: "Sécurisation d'une RESTful API : Authentification et autorisation d'accès aux opérations d'une RESTful API via JWT & bcrypt"
---
...
<InternalPageMenu>
  ...
  <InternalPageMenuItem> Exercice 3.1 : Authentification & autorisation d'opérations </InternalPageMenuItem>
  <InternalPageMenuItem> 🍬 Exercice 3.2 : une autre API sécurisée </InternalPageMenuItem> 
...
Dans ce nouveau tutoriel, nous allons continuer le développement de l'API [services](https://github.com/e-vinci/ts-demos/tree/main/back/api/services) pour ajouter des opérations permettant l'authentification et l'autorisation d'utilisateurs.

Veuillez créer un nouveau projet nommé **`auths`** (dans votre repo git **`/web2/tutorials/pizzeria/api/auths`**) sur base d'un copié/collé du tutoriel [services](https://github.com/e-vinci/ts-demos/tree/main/back/api/services).
...
```ts numbered highlighting="7,13,23,29"
import { Router } from "express";
import { PotentialUser } from "../types";
import { login, register } from "../services/users";
const router = Router();
...
# <InternalPageTitle> Exercice 3.1 : Authentification & autorisation d'opérations </InternalPageTitle>

Vous allez sécuriser la RESTful API de **myMovies**.

Veuillez créer un nouveau projet ans votre repo git dans **`/exercises/3.1`** en clonant le boilerplate du cours offrant l'authentification et l'autorisation JWT : [jwt-ts-api-boilerplate](https://github.com/e-vinci/jwt-ts-api-boilerplate). Dans ce boilerplate, vous trouverez déjà un service **`users`** qui gère les utilisateurs, ainsi que des routes pour les opérations de **`login`** et de **`register`**.

Dans ce projet, dans un premier temps, sans utiliser JWT, veuillez intégrer le code utile de vos opérations (router & service de **`films`**, requêtes HTTP pour tester les opérations sur les **`films`**) de votre **`/exercises/1.8`**. 

Une fois que vous opérations sur les films sont bien intégrées, veuillez planifier de nouvelles opérations et quelles opérations protéger par JWT. 

Pour les nouvelles opérations, vous devez pouvoir ajouter des commentaires sur un film et lire tous les commentaires associés à un film.

Pour la protection des opérations, vous ne devez pas gérer de rôles, mais simplement vérifier que l'utilisateur est authentifié.

Veuillez d'abord identifier toutes les opérations mises à disposition par votre API (y compris l'authentification), ainsi que si celles-ci sont protégées par JWT, au sein du fichier **`README.md`** (fichier Markdown) de votre projet. Voici un exemple de comment nous vous recommandons de documenter votre API, sous forme de tableau : 
...
Une fois la documentation de votre API prête, vous pouvez commencer par protéger les opérations existantes et les tester avec Rest Client.

Une fois que tout est fonctionnel, vous pouvez implémenter les nouvelles opérations de gestion de commentaires et les protéger par JWT.

Veuillez vérifier que vos opérations sont bien protégées et fonctionnelles en les testant avec Rest Client.

Quand un prototype d'api est finalisé et testé, veuillez faire un **`commit`** de votre code avec comme message : **`new: ex3.1`**.

#### 🤝 Tips
Comment créer un tableau dans un fichier Markdown (pour **`README.md`**) ?  
Voici deux options :
- Soit vous utiliser des **`|`** pour délimiter les cellules et des **`|---|`** pour séparer les headers du corps du tableau. 
...
# <InternalPageTitle> 🍬 Exercice 3.2 : une autre API sécurisée </InternalPageTitle>
...
Veuillez créer un nouveau projet dans votre repo git dans **`/exercises/3.2`** sur base du boilerplate : [jwt-ts-api-boilerplate](https://github.com/e-vinci/jwt-ts-api-boilerplate).
...
Veuillez créer un nouveau projet nommé **`/back/api/cookies`** sur base d'un copié/collé du projet [auths](https://github.com/e-vinci/ts-demos/tree/main/back/api/auths)).
