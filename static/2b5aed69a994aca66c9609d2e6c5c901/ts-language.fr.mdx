...
Dans cet exemple, TypeScript utilise des assertions de type avec `as` pour forcer le type des propriétés de `apiResponse` à `number` et `string` respectivement. Cela garantit que `user` est de type `Person` et évite les erreurs de type potentielles. 
